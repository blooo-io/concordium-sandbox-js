...
const signSimpleTransferWithMemo = async () => {
  try {
    ...
    const { signature } = await ccd.signTransferWithMemo(tx, "1105/0/0/0/0/2/0/0");
...
const signUpdateContract = async () => {
...
const signUpdateCredential = async () => {
...
const signPLT = async () => {
  try {
    ...
    const tx = {
      ...
      payload: {
        operation: "transfer",
        operationData: "81A1687472616E73666572A266616D6F756E74C482211904C769726563697069656E74D99D73A201D99D71A10119039703582020A845815BD43A1999E90FBF971537A70392EB38F89E6BD32B3DD70E1A9551D7"
      }
    };
...
document.getElementById("simple-transfer").onclick = async function () {
  ...
  const signature = await signSimpleTransfer();

  document.getElementById("simple-transfer-input").value = signature;
};

document.getElementById("simple-transfer-with-memo").onclick = async function () {
  //Building transaction
  const signature = await signSimpleTransferWithMemo();
  document.getElementById("simple-transfer-with-memo-input").value = signature;
};
document.getElementById("transfer-with-schedule").onclick = async function () {
  //Building transaction
  const signature = await signTransferWithSchedule();
  document.getElementById("transfer-with-schedule-input").value = signature;
};
document.getElementById("transfer-with-schedule-memo").onclick = async function () {
  //Building transaction
  const signature = await signTransferWithScheduleMemo();
  document.getElementById("transfer-with-schedule-memo-input").value = signature;
};

document.getElementById("configure-delegation").onclick = async function () {
  //Building transaction
  const signature = await signConfigureDelegation();
  document.getElementById("configure-delegation-input").value = signature;
};

document.getElementById("configure-baker").onclick = async function () {
  //Building transaction
  const signature = await signConfigureBaker();
  document.getElementById("configure-baker-input").value = signature;
};

document.getElementById("register-data").onclick = async function () {
  //Building transaction
  const signature = await signRegisterData();
  document.getElementById("register-data-input").value = signature;
};

document.getElementById("transfer-to-public").onclick = async function () {
  //Building transaction
  const signature = await signTransferToPublic();
  document.getElementById("transfer-to-public-input").value = signature;
};

document.getElementById("deploy-module").onclick = async function () {
  //Building transaction
  const signature = await signDeployModule();
  document.getElementById("deploy-module-input").value = signature;
};

document.getElementById("init-contract").onclick = async function () {
  //Building transaction
  const signature = await signInitContract();
  document.getElementById("init-contract-input").value = signature;
};

document.getElementById("update-contract").onclick = async function () {
  //Building transaction
  const signature = await signUpdateContract();
  document.getElementById("update-contract-input").value = signature;
};

document.getElementById("update-credential").onclick = async function () {
  //Building transaction
  const signature = await signUpdateCredential();
  document.getElementById("update-credential-input").value = signature;
};

document.getElementById("credential-deployment").onclick = async function () {
  //Building transaction
